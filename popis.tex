...
kde x je vzdálenost, ve které se míček nachází ve stejné výšce, z jaké byl vystřelen v metrech. L je aerodynamická délka opět v metrech. Dále je zde $\varphi$, což je počáteční úhel elevace. V neposlední řadě vztah obsahuje počáteční rychlost košíku $v_0$ a maximální pádovou rychlost $v_{inf}$. Obě rychlosti se dosazují v $m\cdot s^{-1}$. Změřené údaje popisující možnosti stroje jsou uvedeny v kapitole \ref[mereni].
